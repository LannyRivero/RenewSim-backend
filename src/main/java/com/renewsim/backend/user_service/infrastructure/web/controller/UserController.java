...
public class UserController {
    ...
    // POST /users → Create
    // ----------------------------------------------------
    @Operation(summary = "Create user", description = "Requires role ADMIN or scope user:write",
    ...
    @Operation(summary = "Get user by ID",
            description = "Requires role ADMIN, scope user:read or be the owner (@userSec.isOwner)",
            security = @SecurityRequirement(name = "bearerAuth"))
    @GetMapping("/{id}")
    ...
    // ----------------------------------------------------
    @Operation(summary = "Get user by username", description = "Requires role ADMIN or scope user:read",
            security = @SecurityRequirement(name = "bearerAuth"))
    ...
    public ResponseEntity<OperationResponse<UserResponse>> getByUsername(@RequestParam String username) {
        ...
        return ResponseEntity.ok(ApiResponseFactory.ok(results.content().get(0), "User retrieved successfully"));
    }

    // ----------------------------------------------------
    // GET /users/by-email → Filter by email
    // ----------------------------------------------------
    @Operation(summary = "Get user by email", description = "Requires role ADMIN or scope user:read",
            security = @SecurityRequirement(name = "bearerAuth"))
    @GetMapping("/by-email")
    @PreAuthorize("hasAuthority('SCOPE_user:read') or hasRole('ADMIN')")
    ...
    // ----------------------------------------------------
    @Operation(summary = "Get internal credentials",
            description = "Requires role SERVICE_AUTH (internal use between microservices)",
            security = @SecurityRequirement(name = "bearerAuth"))
    ...
    // ----------------------------------------------------
    @Operation(summary = "List users", description = "Requires role ADMIN or scope user:read",
            security = @SecurityRequirement(name = "bearerAuth"))
    @GetMapping
    @PreAuthorize("hasAuthority('SCOPE_user:read') or hasRole('ADMIN')")
    ...
    // ----------------------------------------------------
    @Operation(summary = "Check if user exists", description = "Requires role ADMIN or scope user:read",
            security = @SecurityRequirement(name = "bearerAuth"))
    @GetMapping("/exists")
    @PreAuthorize("hasAuthority('SCOPE_user:read') or hasRole('ADMIN')")
