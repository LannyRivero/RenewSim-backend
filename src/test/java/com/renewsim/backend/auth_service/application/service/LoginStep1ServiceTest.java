...
class LoginStep1ServiceTest {
        ...
        void execute_validCredentials_generatesOtpAndSendsEmail() {
                ...
                LoginStep1ResultDTO result = service.execute(new LoginStep1Command("john@example.com", "secret"));
        ...
        void execute_unknownEmail_returnsGenericWithoutEmail() {
                when(userAccountGateway.findByEmail("unknown@example.com")).thenReturn(Optional.empty());

                LoginStep1ResultDTO result = service.execute(new LoginStep1Command("unknown@example.com", "secret"));
        ...
        void execute_disabledUser_returnsGenericWithoutEmail() {
                when(userAccountGateway.findByEmail("jane@example.com")).thenReturn(Optional.of(disabledUser));

                LoginStep1ResultDTO result = service.execute(new LoginStep1Command("jane@example.com", "secret"));
        ...
        @Test
        @DisplayName("password incorrecta → respuesta genérica, sin OTP ni email")
        void execute_wrongPassword_returnsGenericWithoutEmail() {
                when(userAccountGateway.findByEmail("john@example.com")).thenReturn(Optional.of(activeUser));
                when(passwordEncoder.matches("wrongpass", activeUser.passwordHash())).thenReturn(false);

                LoginStep1ResultDTO result = service.execute(new LoginStep1Command("john@example.com", "wrongpass"));
        ...
        void execute_validCredentials_emailPortReceivesCorrectArgs() {
                when(userAccountGateway.findByEmail("john@example.com")).thenReturn(Optional.of(activeUser));
                when(passwordEncoder.matches("secret", activeUser.passwordHash())).thenReturn(true);
