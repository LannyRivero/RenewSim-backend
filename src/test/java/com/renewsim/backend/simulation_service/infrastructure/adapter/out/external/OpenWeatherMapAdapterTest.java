...
class OpenWeatherMapAdapterTest {
    ...
    @Test
    @DisplayName("fetchClimateData returns fallback climate on upstream failure")
    void fetchClimateDataReturnsFallbackClimateOnFailure() {
        ...
        server.expect(requestTo("https://api.openweathermap.org/data/2.5/weather?lat=-32.8895&lon=-68.8458&units=metric&appid=test-key"))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withServerError());

        var climate = adapter.fetchClimateData(-32.8895, -68.8458);

        assertThat(climate.irradiance()).isEqualTo(800);
        assertThat(climate.wind()).isEqualTo(5);
        assertThat(climate.source()).isEqualTo("OPENWEATHERMAP");
        server.verify();
    }
